...
//    private static final String BASE_URI = "http://localhost:4564/api/";
	...
	public WebServiceTaskRepository(String baseApiUri) {
	...
	public void add(Task task) {
		try {
			final URL url = new URL(baseApiUri + "task");
	...
	public Task removeByListNumber(int listNumber) {
		...
		try {
			final URL url = new URL(baseApiUri + "task/" + listNumber);
			final HttpURLConnection connection = (HttpURLConnection) url.openConnection();
	...
	public Task completeByListNumber(int listNumber) {
		...
		try {
			final URL url = new URL(baseApiUri + "task/" + listNumber + "/complete");
			final HttpURLConnection connection = (HttpURLConnection) url.openConnection();
			connection.setRequestMethod("POST");
			connection.setRequestProperty("Content-Type", "application/json");
			connection.setReadTimeout(TIMEOUT);
